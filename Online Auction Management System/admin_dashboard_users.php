...
<body>
    ...
    <section class="container-2">
        <div class="user-table-container">
            <form class="search-container" action="admin_dashboard_users.php" method="POST">
                <input type="text" placeholder="&#xf002;  BuyerID, Username, Name" name="search-input">
                <select name="filters" id="filters">
                    <option value="">Select Filter</option>
                    <option value="View By Top Users">View By Top Users</option>
                    <option value="View By Less Active Users">View By Less Active Users</option>
                </select>
            ...
            <table class="user-table">
                ...
                <?php
                    ...
                    if (isset($_POST['search-input'])) {
                        ...
                        if ($search != '') {
                            $where = "WHERE Buyer.BuyerID = '$search' OR Buyer.Username = '$search' OR Fname = '$search' OR Lname = '$search'";
                        }
                    }
                    // var_dump($where);

                    $query = "SELECT Buyer.BuyerID, Username, Fname, Lname, Street, AptNo, City, State, ZipCode, Email, PhoneNo, COUNT(Bids.BuyerID) AS NoOfBids
                    FROM Buyer
                    LEFT JOIN Bids
                    ON Buyer.BuyerID = Bids.BuyerID ".$where
                    ." GROUP BY BuyerID;";
                    if (isset($_POST['filters'])) {
                        $filter = $_POST['filters'];
                        switch ($filter) {
                            case 'View By Top Users':
                                $query = "SELECT Buyer.BuyerID, Username, Fname, Lname, Street, AptNo, City, State, ZipCode, Email, PhoneNo, COUNT(Bids.BuyerID) AS NoOfBids
                                FROM Buyer
                                LEFT JOIN Bids
                                ON Buyer.BuyerID = Bids.BuyerID ".$where
                            ...
                            case 'View By Less Active Users':
                                $query = "SELECT Buyer.BuyerID, Username, Fname, Lname, Street, AptNo, City, State, ZipCode, Email, PhoneNo, COUNT(Bids.BuyerID) AS NoOfBids
                                FROM Buyer
                                LEFT JOIN Bids
                                ON Buyer.BuyerID = Bids.BuyerID ".$where
                ...
                <tr>
                    ...
                    <td><a href="http://localhost/apna-Project/admin_dashboard_users_profile.php?buyer_id=<?php echo $row['BuyerID']; ?>"><button><i class="fa-regular fa-eye"></i> View</button></a></td>
                </tr>
                <?php 
                    } 
                ?>

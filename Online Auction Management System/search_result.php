    ...
    <section class="container-2">
        <div class="item-container">
            <?php
                ...
                //     $where = ' WHERE ';
                // }
                // // $query2 = "SELECT * FROM `Auction`.`Items` $where $category;";

                // $sub = $_GET['search-input'];
                // while ($row1 = mysqli_fetch_array($result1)) {
                //     $str = $row1['Keyword'];
                //     $match = stripos($str, $sub);

                //     if ($match == true) {
                //         $resultFlag = true;
                //         $query2 = "SELECT * FROM `Auction`.`Items` WHERE ItemID = $row1[ItemID];";
                ...
                if ($sub !== '') {
                    ...
                    while ($row1 = mysqli_fetch_assoc($result1)) {
                        ...
                        // var_dump($match);
                        if ($match !== false) {
                            $query2 = "SELECT * FROM `Auction`.`Items` $where $category $and ItemID = $row1[ItemID];";
                            $result2 = mysqli_query($con, $query2);
                            $row2 = mysqli_fetch_assoc($result2);
                            if ($row2 > 0) {
                                $resultFlag = true;
                                $currentPrice = $row2['StartBidAmount'];
                                $query4 = "SELECT * FROM Bids 
                                WHERE ItemID = $row2[ItemID] AND BidAmount = (
            ...
            ?>
            <a href="http://localhost/apna-Project/item_details.php?item_id=<?php echo $row2['ItemID']; ?>">
            <div class="item-card">
                ...
                <div class="info">
                    ...
                    <p class="price">₹<?php echo $row2['StartBidAmount']; ?></p>
                    <?php if (new DateTime() <= new DateTime($row2['AuctionStart'])) { ?>
                    <p class="time">Beginning soon...</p>
                    <?php } elseif (new DateTime() > new DateTime($row2['AuctionStart']) && new DateTime() < new DateTime($row2['AuctionEnd'])) { ?>
                    <p class="time">In progress</p>
                    <?php } else { ?>
                    <p class="time">Over</p>
                    <?php } ?>
                    <!-- <p class="time">Ends at <?php echo (new DateTime($row2['AuctionEnd']))->format("d/m/Y"); ?></p> -->
                </div>
            </div>
            </a>
            <?php
